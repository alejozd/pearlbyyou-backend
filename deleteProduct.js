...
async function deleteProduct(id) {
  ...
  try {
    ...
    console.log(
      `✅ Registros de imágenes para el producto ${id} eliminados de la DB.`
    );
    ...
    if (deletedCount === 0) {
      console.warn(
        `⚠️ No se encontró ningún producto con ID ${id} para eliminar.`
      );
    } else {
      console.log(`✅ Producto con ID ${id} eliminado de la DB.`);
    }
